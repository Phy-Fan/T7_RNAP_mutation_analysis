...
def calculate_salt_bridges(site, residue, protein_name, distance_cutoff=4.5, debug=False):
    try:
        cmd.reinitialize()
    ...
    if residue in basic_residues:
        ...
        opposite_sels = []
        for res, atoms in charged_atoms.items():
            if res in ['ASP', 'GLU']:
                atoms_sel = '+'.join(atoms)
                opposite_sels.append(f"(resn {res} and name {atoms_sel})")
        opposite_sel = ' or '.join(opposite_sels)
...
def process_pdb_files(protein_list, debug=False):
    for p in protein_list:
        ...
        file_names = [f for f in os.listdir(folder_path) 
                     if "relaxed" in f and "unrelaxed" not in f]
        ...
        for file_name in file_names:
            ...
            wt_salt_bridges = calculate_salt_bridges(site, o_res, wt_protein_name, debug=debug)
            mutant_salt_bridges = calculate_salt_bridges(site, m_res, protein_name, debug=debug)
        ...
        columns = [
            'mutant', 'original_res', 'mutated_res', 'position',
            'wt_salt_bridges', 'mutant_salt_bridges', 'change_in_salt_bridges'
        ]
...
if __name__ == "__main__":
    protein = ['T7']
    process_pdb_files(protein, debug=False)
